...
class diagnostic():
    ...
    def corner_plot(self,fusion):
        ...
        samples = fusion.reshape(-1,6)

        print(samples.shape)


        f0, A1, A2, A3,zt,zs = map(lambda v: (v[1], v[2] - v[1], v[1] - v[0]),
                                     zip(*np.percentile(samples, [16, 50, 84],
                                                        axis=0)))

        print(f0[0],A1[0],A2[0],A3[0],zt[0],zs[0])

        fig = corner.corner(samples, labels=name,truths=[f0[0],A1[0],A2[0],A3[0],zt[0],zs[0]])
...
#0.13378627312 -0.057777279267 569222 -0.916024608899 1.41650849458 0.993917045235
...
pkl_file = open('Simultaneously_EMCEE.pkl', 'rb')
result = pickle.load(pkl_file)
pkl_file.close()
...
model.corner_plot(fusion=result)
